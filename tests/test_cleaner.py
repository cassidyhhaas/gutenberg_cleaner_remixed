...
class BoundaryTests(unittest.TestCase):
    ...
    def test_rejects_an_unresolved_boundary(self):
        ...
        self.assertEqual(context.exception.metadata["boundary_status"], "unresolved")
...
class NormalizationTests(unittest.TestCase):
    ...
    def test_rejects_invalid_utf8(self):
        with self.assertRaises(UnicodeDecodeError):
            super_cleaner(b"\xff" + START.encode() + b"\nBody\n" + END.encode())

    def test_reflows_prose_and_preserves_structured_text(self):
        prose = (
            "This fixed-width prose line contains enough text for safe paragraph reflow.\n"
            "The next fixed-width prose line also contains enough text for reflow.\n"
            "The final line completes the paragraph."
        )
        verse = "The moon is high\nThe river is still\nNight holds its breath"
        table = "Name   Count   Place\nJane   2       Bath\nJohn   4       York"
...
class RemovalAndMetadataTests(unittest.TestCase):
    ...
    def test_removes_a_contents_section_only_when_headings_repeat(self):
        ...
        self.assertEqual(result.removed_sections[0]["type"], "table_of_contents")
    ...
    def test_extracts_metadata_before_removal(self):
        ...
        self.assertEqual(result.metadata["source_url"], "https://www.gutenberg.org/ebooks/123")
        self.assertEqual(result.metadata["cleaner_version"], CLEANER_VERSION)
        self.assertEqual(result.metadata["raw_sha256"], hashlib.sha256(book.encode()).hexdigest())
    ...
    def test_moves_production_credits_to_audit_output(self):
        body = "Produced by Jane Doe and Distributed Proofreaders\n\nTEST BOOK\n\nBody"

        result = clean_book(make_book(body))

        self.assertNotIn("Produced by", result.text)
        self.assertEqual(result.removed_sections[0]["type"], "production_credits")
    ...
    def test_adds_missing_title_and_author_headings(self):
        prefix = "Title: Test Book\nAuthor: Jane Doe\nRelease Date: Today [EBook #123]"

        cleaned = super_cleaner(make_book("CHAPTER I\n\nBody", prefix))

        self.assertTrue(cleaned.startswith("Test Book\nBy Jane Doe\n\nCHAPTER I"))
    ...
    def test_removes_terminal_publisher_advertisements(self):
        ...
        self.assertEqual(result.removed_sections[0]["type"], "publisher_advertisements")
